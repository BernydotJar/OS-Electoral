...
The repository now contains a real local application foundation: a versioned FastAPI runtime, fail-closed OIDC token verification, typed configuration, an initial SQLAlchemy/Alembic PostgreSQL schema, transaction-local tenant scope, forced row-level-security policies, a non-root container, a hermetic Compose stack, locked dependencies and pinned CI definitions. These are meaningful local proofs, but they are not deployed or operational production evidence.
...
- GitHub Actions and runtime container references used in the new foundation are pinned to immutable commits/digests. Dependabot update definitions, CodeQL, dependency audit, secret scan, actionlint and disposable Compose-stack E2E jobs exist locally.
...
- The new CI has not yet run on GitHub. Main branch protection/rulesets were absent when observed, and repository settings do not require Action SHA pinning.
- CodeQL and secret/dependency jobs are definitions until a reviewed GitHub run proves execution and findings are triaged.
...
| CI definition | `.github/workflows/campaignos-ci.yml` passes actionlint | Unobserved until GitHub executes it |
...
Later integration runs `29660653755` and `29662896729` passed their checked commits, but do not rewrite those six records.
...
1. Publish the branch for review, obtain a green run of every new CI job, triage findings, and configure protected-main rules without weakening required checks.
