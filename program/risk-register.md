...
|---|---|---|---|---|---|---|
| RISK-PLATFORM-001 | CRITICAL | REMEDIATION_IN_PROGRESS | A plan-only Terraform baseline and test-only recovery verifier exist, but no verified AWS dev/staging/production runtime, live state, managed backup/PITR, deployed telemetry or accepted managed recovery evidence exists. | Protected `main@a12bed0771299ef8c7bc611b69b1b8db1c01d968` passed CI `30424626008`, including PostgreSQL recovery artifact `8713127644`; the temporary authenticated review environment was stopped and is not managed-environment evidence. | Human-authorize an isolated dev/staging account and cost envelope; verify live state/plan/apply, encrypted managed backups, PITR, telemetry, smoke/security and RPO/RTO before any production gate. | WS-12 / WS-14 |
| RISK-ABUSE-001 | HIGH | OPEN | The API has exact authorization but no tenant/principal application-layer rate limiter, leaving authenticated abuse and resource-exhaustion paths unbounded. | Graph Harness SHIP spec `C3-SEC-002` defines server-owned policy classes, PostgreSQL atomicity, sanitized 429 responses and adversarial tests; implementation is not approved. | Human-approve the spec, implement the bounded limiter, prove concurrency/cross-tenant behavior and retain staging load/WAF as separate gates. | WS-03 / WS-05 / WS-13 |
...
| RISK-FRONT-001 | MEDIUM | REMEDIATION_IN_PROGRESS | The local exact-authorized shell supports guided intake, candidate evidence and team operations, but production identity, complete candidate editing, independent strategist acceptance and the current command-hierarchy QA repair remain unfinished. | Server-only token handling, exact campaign grants, same-origin UI routes, bilingual contracts, local functional journeys, WCAG automation and `C3-FRONT-008R1` fail-closed readiness/hierarchy tests are present. | Complete exact-head browser/CI review for the QA repair, then add governed candidate editing, live identity/tenant selection and independent accessibility/operational acceptance before any deployment claim. | WS-03 / WS-06 / WS-07 |
