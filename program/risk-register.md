...
| RISK-AUDIT-001 | MEDIUM | REMEDIATION_IN_PROGRESS | Non-owner mutation is denied on six append-only journals, but database owner/member break-glass can still rewrite/delete rows and no external anchor or restore proof exists. | Tenant-serialized hash chain, revision `20260721_0011`, trigger introspection and constrained-role update/delete denial pass twice in PostgreSQL. | Add privileged-access workflow/alerts, external integrity anchoring, legal retention and restore verification. | WS-04 / WS-13 / WS-14 |
