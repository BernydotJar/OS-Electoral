...
| PostgreSQL persistence | Identity/tenancy, campaign create/read/update, idempotency, workspace, audit/outbox/readiness, identity lifecycle and guided intake have constrained-role PostgreSQL proof; the published stack through identity is CI-green | PARTIAL | Broader domain adapters, managed-role rotation and RDS/staging transaction evidence |
| Database migrations | Alembic downgrade/upgrade/check reaches revision `20260721_0005`; lifecycle and guided-intake tables, forced RLS, constraints and indexes are exercised on a disposable PostgreSQL database twice | PARTIAL | Reviewed release policy, representative legacy-data rehearsal and staging forward/compatibility evidence |
| Versioned REST API | `/api/v1` additionally exposes invitation, session, membership-revocation, time-bound support and guided-intake start/read/update routes with exact authorization, idempotency/version preconditions, safe errors and typed OpenAPI | PARTIAL | Broader bounded contexts, rate controls, customer identity UI and deployed verification |
| Dynamic frontend shell | Draft PR `#89` at `437b469` passes exact-head CI/browser/WCAG gates; the current worktree adds an exact-contract read-only guided-intake roadmap and remains server-rendered, ES/EN and fail-closed | PARTIAL | Publish onboarding CI, live OIDC/session integration, trusted tenant selection, complete authorized journeys, human review, dev/staging deployment and user acceptance |
...
| PR CI | PRs `#72`, `#73`, `#83` are merged; draft stack `#84` → `#89` has correct bases and green exact-head checks, including frontend repair run `29856835515` | PARTIAL | Publish C3-IAM-002, obtain its exact-head CI and human review; enforce required checks on protected main |
...
| Branch protection | Authenticated API confirms `main` is unprotected, rulesets are empty, all Actions/workflows are allowed and SHA pinning is not required | NOT_IMPLEMENTED | Human-configured protected-main ruleset with reviews, required checks and restricted immutable Actions policy |
...
| Required documentation tree | Campaign create/readiness, identity lifecycle, guided intake and frontend runtime API/product/testing docs exist; broader product and operations paths remain incomplete | PARTIAL | Complete all required docs, ownership and stale-content validation |
