...
| Training Academy | Static team guidance plus complete governed bilingual SHIP requirements/design/tasks; implementation is not authorized | NOT_IMPLEMENTED | Human approval, learning paths, content governance, durable assignments/completions, assessments and exact-head evidence |
...
| Rate limiting and abuse protection | `C3-SEC-002` locally covers 41 protected routes with server-owned classes, atomic PostgreSQL counters, forced RLS, fail-closed errors, bounded telemetry and concurrency/rollover/cleanup tests | PARTIAL | Exact-head hosted CI, managed staging load, cleanup scheduling, edge DDoS/WAF and independent production acceptance |
...
| PR CI | Cumulative PR `#126` is merged by rebase; PRs `#118`–`#125` are closed as superseded with evidence preserved | PASS | Preserve protected-main controls and continue exact-head feature evidence |
| Main CI | `main@a12bed0771299ef8c7bc611b69b1b8db1c01d968` passed post-merge CI `30424626008`, including PostgreSQL/RLS, recovery, API/browser, CodeQL, supply chain and constrained stack | PASS | Run controlled staging deployment and post-deploy verification after separate authorization |
...
| Release readiness audit | Machine-readable `DENY_RELEASE` record, explicit historical supersession, cumulative merge receipt and green post-merge main CI | PARTIAL | Obtain managed staging evidence, close technical gaps and collect all independent approvals |
