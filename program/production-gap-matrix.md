...
| Dynamic frontend shell | Published Next.js 16/React 19 server-rendered shell, server-only typed API client, runtime JSON validation, fail-closed states, ES/EN, responsive Premium Slate foundations and synthetic read-only demo mode pass local build/browser/image gates | PARTIAL | Live OIDC/session integration, trusted tenant selection, complete authorized journeys, draft PR/current-head CI/review, dev/staging deployment and user acceptance |
...
| PR CI | PRs `#72`, `#73` and `#83` merged with green recorded checks; draft stack `#84` -> `#85` -> `#86` is green; readiness, campaign-create and dynamic-shell branches are published, while the frontend head has zero PRs/runs | PARTIAL | Create the frontend draft PR under authenticated access, obtain green checks, human review and required-check enforcement on protected main |
