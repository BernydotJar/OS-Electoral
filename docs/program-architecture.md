...
The manifest identifies C2-SAAS-001B as executable: define explicit append-only persistence and audit adapter contracts without adding a database, deployment or credentials. C2-AI-001A may follow with evidence-grounded extraction and citation contracts that operate without external model credentials.
