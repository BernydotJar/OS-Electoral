...
5. **C3-FRONT-015-F5 — HIGH — RESOLVED.** PR #182 first exact head `46c41bf25b4a26f71cd11da0ca440dcf1e4b5dcd` ran CampaignOS CI `32002418220`. PostgreSQL/RLS, Terraform plan-only, stack E2E, quality, dependency audit, recovery, supply chain, secret scan, CodeQL and Runtime Visual all passed, but the API-backed functional onboarding journey failed before mutations because the live shell did not render. The backend correctly returned HTTP 409 with problem code `CAMPAIGN_NOT_READY` when the Operations roadmap was not yet available because prerequisites were incomplete. `shell-view-model` recognized 404 as `NOT_STARTED` and 503 as dependency unavailable, but rethrew this prerequisite 409 and converted the whole shell to unavailable. The Fixer adds a bounded read-state classifier: 404 and only `409 CAMPAIGN_NOT_READY` map to Operations `NOT_STARTED`, 503 maps to dependency unavailable, and unrelated 409 codes such as `ROADMAP_CONFLICT` and `IDEMPOTENCY_CONFLICT` remain fail-closed. Regression tests cover each classification. The final frontend verifier passes **55 test files / 250 tests**, ESLint, TypeScript, optimized build and audit 0. Hosted exact-head revalidation is required before merge.

6. **C3-FRONT-015-F6 — MEDIUM — RESOLVED.** The second PR #182 hosted head confirmed F5 was repaired and the functional journey progressed through Operations writes and War Room snapshot creation. It then failed a persistence assertion that searched for the learning title as a visible text node. Hosted API evidence proves the learning write returned PATCH 200, the snapshot returned POST 201, and the latest snapshot read returned 200. The persisted learning is rendered as the value of an editable `input[name="title"]`, so `get_by_text` was the wrong assertion. The verifier now opens the learning section, requires the persisted plus new forms, asserts the persisted input value, reloads, and asserts the same value again. The persistence check is stronger rather than removed.


## Hosted exact-head verification

PR #182 implementation/Fixer head `55153857c59d17ba11c1e608d8f506b47b3517a3` passed:

- CampaignOS CI #270 (`32003359525`): **SUCCESS**;
- API-backed functional onboarding journey: **SUCCESS**, including Candidate → Team current-session role coverage → Strategy `DECIDED_INTERNAL` → Operations roadmap → explicit human decision → learning → War Room snapshot and reload persistence;
- PostgreSQL migrations/RLS, recovery, stack E2E, locked quality, dependency audit, supply-chain, secret scan, Terraform plan-only and CodeQL: **SUCCESS**;
