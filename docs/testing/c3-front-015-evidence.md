...
6. **C3-FRONT-015-F6 — MEDIUM — RESOLVED, HOSTED REVALIDATION PENDING.** The second PR #182 hosted head confirmed F5 was repaired and the functional journey progressed through Operations writes and War Room snapshot creation. It then failed a persistence assertion that searched for the learning title as a visible text node. Hosted API evidence proves the learning write returned PATCH 200, the snapshot returned POST 201, and the latest snapshot read returned 200. The persisted learning is rendered as the value of an editable `input[name="title"]`, so `get_by_text` was the wrong assertion. The verifier now opens the learning section, requires the persisted plus new forms, asserts the persisted input value, reloads, and asserts the same value again. The persistence check is stronger rather than removed.
