...
The initial migration now creates composite tenant/campaign/workspace relationships, enables and forces RLS on tenant-owned tables, and the integration test proves two-tenant campaign visibility plus cross-tenant write denial under a non-superuser, non-`BYPASSRLS` role. This is a bounded local proof. It does not yet cover application repositories, multiple memberships, workers, exports, object keys, caches, search/pagination, staging configuration or operational role rotation, so the production tenant-isolation gate remains `PARTIAL`.
